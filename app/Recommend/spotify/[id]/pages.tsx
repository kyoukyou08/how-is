...
import { getTrackInfo, getTrackFeatures } from "../getFeatures";
...
export default function Spo({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // console.log(params.id);
        const trackData = await getTrackInfo(params.id, accessToken);
        const trackFeature = await getTrackFeatures(params.id, accessToken);
    ...
    <div className="flex justify-center h-screen">
      {trackInfo && feature ? (
        <div>
          ...
          {/* 表カード */}
          <div>
            <div className="mt-8 text-4xl font-bold text-center text-gray-800">
              Audio Features
            </div>
              ...
              <table className="w-full text-sm text-left rtl:text-right text-gray-100">
                <tbody>
                  <tr className="bg-gray-500 border-b border-gray-400">
                    <th
                      scope="row"
                      className="px-6 py-4 font-medium bg-gray-500 text-gray-50 whitespace-nowrap"
                    >
                      BPM
                    </th>
                    <td className="px-6 py-4">{feature.tempo}</td>
                  </tr>
                  <tr className="bg-gray-500 border-b border-gray-400">
                    <th
                      scope="row"
                    ...
                    <td className="px-6 py-4">{getKeyName(feature.key)}</td>
                  </tr>
                  <tr className="bg-gray-500 border-b border-gray-400">
                    <th
                      scope="row"
                  ...
                  <tr className="bg-gray-500 border-b border-gray-400">
                    ...
                    <td className="px-6 py-4">{feature.loudness} dB</td>
                  </tr>
                </tbody>
              </table>
            </div>
                  ...
                  {data.labels.map((label: string, index: number) => (
                    ...
                    >
                      ...
                      <td className="p-4 pr-40 relative">
                        <div
                          className="absolute inset-0 bg-gray-600 rounded-r-lg"
                          style={{
                            width: `${
                              (data.datasets[0].data[index] || 0) * 100
                            }%`,
                          }}
                        ></div>
                      </td>
